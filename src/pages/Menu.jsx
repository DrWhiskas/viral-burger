...
export default function Menu() {
	...
	return (
		<>
			...
			<div className="menu">
				{burgers.map((burger, key) => (
					<MenuForm
						...
						menuDescription={burger.description}
						menuRecipe={burger.burger.ingredients}
						menuPrice={burger.price}
						addArticle={() => addToCart(burger.title, burger.price)}
					/>
				))}
			</div>
			<h1 className="menu__title title">Nos Burgers</h1>
			<div className="menu">
				{burgers.map((burger, key) => (
					<MenuForm
						key={burger.id}
						menuTitle={burger.burger.name}
						menuImage={burger.burger.image}
						menuRecipe={burger.burger.ingredients}
						menuPrice={burger.burger.price}
						addArticle={() =>
			...
			{isOpenModal ?? (
				<Modal message="Merci d'avoir commandé !" closeModal={closeModal} />
			)}
		</>
	);
}
